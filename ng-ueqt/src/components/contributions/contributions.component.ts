...
import { UTooltipDirective } from '../tooltip';
        ...
        <tr *ngFor="let week of ['日', '一', '二', '三', '四', '五', '六'];index as y;">
            ...
            <ng-container *ngFor="let nWeek of weekArray;index as x;">
                <td [ngClass]="values[y*this.weekNumber+x].class">
                    <a *ngIf="values[y*this.weekNumber+x].date.getFullYear() === uOptions.year && values[y*this.weekNumber+x].value !== 0"
                      [ngStyle]="{'background-color': values[y*this.weekNumber+x].color}" [uTooltip]="valueTip"></a>
                    <span *ngIf="values[y*this.weekNumber+x].date.getFullYear() === uOptions.year
                      && values[y*this.weekNumber+x].value === 0"></span>
